...
Much of our focus in this project will be on DMV representatives, and (gracefully) Mitchell, Van Drew, and Amash were all from other This means we can redo the join now without worrying about the remaining many-to-many issues for questions that pertain specifically to DMV reps. If we need to include any national-level analyses we'll reapproach those records then. (Maybe re-code their party as "M" for "multiple" throughout their tenures in the House?)
