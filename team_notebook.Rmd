...
totals_dmv_spend_pct_change <- totals_dmv_spend %>%
  ...
  mutate(pct_change = c((amount - lag(amount)) / lag(amount) * 100)) %>%
...
national_wages_pct_change <- national_wages %>%
  mutate(pct_change = c((avg_salary - lag(avg_salary)) / lag(avg_salary) * 100)) %>%
...
ggplot(wage_comparison_pct_change_df, aes(x = year, y = pct_change, color = data_source, group = data_source)) +
  geom_line() +
  geom_point() +
  labs(title = "Percent Change Over Years by Data Source",
       x = "Year",
       y = "Percent Change",
       color = "Data Source") +
  theme_minimal()
