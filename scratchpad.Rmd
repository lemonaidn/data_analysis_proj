...
# We see office years in here from 2011 to 2023. That can't be right, let's filter them out. 

house_spend_18_22 <- house_spend_18_22 |> 
  filter(office_year<=2022)|> 
  filter(office_year>=2018)

# That looks good to work with. 
...
house_members <- house_spend_18_22 %>%
...
house_spend_18_22 <- house_spend_18_22 |> 
  left_join(house_members)

%>%
  summarise(annual_avg_per_member = annual_total / members)
...
# So on average, DMV reps spent less every year than the House as a whole did, on average $17,073.92 less per member each year. 
```
...
# Hypothesis confirmed that they spend substantially less on travel than other members, on average more $32,870.38 less annually per member. But they only spend ~$17K less than others overall, meaning that they are overspending somewhere else. 
