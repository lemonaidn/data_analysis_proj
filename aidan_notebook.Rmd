...
# First, calculate avg spend for all members

house_spend_18_23 <- house_spend_18_23 %>%
  filter(sort_sequence == "SUBTOTAL") %>%
  select(-organization_code, -program_code, -budget_object_class, -data_source, -document, -vendor_id, -payee, -start_date, -end_date, -budget_object_code, -transcode, -recordid, -id, -sort_subtotal_description, -transaction_date, -date) %>%
  # filter out anything that doesn't include a bioguide id so we're not factoring in spend by committees
...
```{r}
# confirm that there are 435 unique members per year
...
house_spend_18_23_totals <- house_spend_18_23 %>%
  filter(purpose == "OFFICIAL EXPENSES OF MEMBERS TOTALS:") %>%
...
house_spend_18_23_totals

```

```{r}

# now do the same for dmv members
...
totals_dmv_spend_totals <- totals_dmv_spend %>%
  ...
  group_by(year) %>%
  summarise(annual_total = sum(amount)) %>%
  mutate(annual_avg_per_member = annual_total / 20)

totals_dmv_spend_totals

```

So on average, DMV reps spent less every year than the House as a whole did. However, it's not by much -- typically only ~20k less.

Now let's break it out by category to test the hypothesis that they probably spend less on transportation than other members do


```{r}

house_spend_18_23_totals_travel <- house_spend_18_23 %>%
...
house_spend_18_23_totals_travel
...
Hypothesis confirmed that they spend substantially less on travel than other members.
...
details_dmv_staff_pay <- dmv_spend_with_member_info %>%
  filter(category == "PERSONNEL COMPENSATION") %>%
  filter(office_year >= 2018) %>%
  # exclude interns, temporary and part-time employees and exclude total/subtotal rows 
  filter(program != "INTERN ALLOWANCES") %>%
  filter(purpose != "PERSONNEL COMPENSATION TOTALS:" & purpose != "TEMPORARY EMPLOYEE" & purpose != "PAID INTERN" & purpose != "PART-TIME EMPLOYEE (OTHER COMPENSATION)" & purpose != "PART-TIME EMPLOYEE (OTHER COMPENSATION)" & purpose != "PART-TIME EMPLOYEE") %>%
  select(-organization_code, -program_code, -budget_object_class, -data_source, -document, -vendor_id, -start_date, -end_date, -budget_object_code, -transcode, -recordid, -id, -sort_subtotal_description, -transaction_date, -date)
