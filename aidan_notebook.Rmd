...
# DATE col values aren't always populated (some rows are and some rows aren't within the same df). can't actually use it to extract quarter and year values. easiest to do those steps manually for each df instead

q4_2022 <- read_csv("data/2022Q4.csv") %>%
  mutate(filing_year = 2022) %>%
  clean_names()
...
chr_col_vec <- c(BIOGUIDE_ID, OFFICE, PROGRAM, CATEGORY, SORT SEQUENCE, TRANSCODE, RECORDID, PAYEE, PURPOSE)
