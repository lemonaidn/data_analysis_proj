...
# Watch out for "subtotal" rows -- need to filter those out when doing calculations to avoid doubing the real total
...
# DATE col values aren't always populated (some rows are and some rows aren't within the same df). can't acually use it to extact quarter and year values. will need to do those steps manually for each df instead
