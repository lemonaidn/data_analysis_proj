...
# Surprisingly, there are more than 435 members each year even after we've filtered out records without a bioguide_id. There's also randomly a member for 2023. Not sure why that is, but we'll need to divide by the numbers above to find our average rather than 435 for every year. 
...
```{r}
# Now let's join to compare side by side. 
...
# So on average, DMV reps spent less every year than the House as a whole did, on average $3,576,305 less per member each year.
```


```{r}
# Now let's use the same process to test the hypothesis that they probably spend less on transportation than other members do. 
...
# Hypothesis confirmed that they spend substantially less on travel than other members, on average more $38,658.13 less annually per member. 
